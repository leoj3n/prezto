...
function set-window-title {
  ...
  zstyle -s ':delorean:circuit:terminal:window-title' format 'title_format' || title_format="%s"
...
function set-tab-title {
  local title_format{,ted}
  zstyle -s ':delorean:circuit:terminal:tab-title' format 'title_format' || title_format="%s"
  zformat -f title_formatted "$title_format" "s:$argv"
...
if zstyle -t ':delorean:circuit:terminal' auto-title \
  && ( ! [[ -n "$STY" || -n "$TMUX" ]] )
then
